...
The CRM MCP server is intra-team tooling for the (Rust) master agent — read-only
Salesforce queries, complementary to the RabbitMQ XML write paths. We bundle it
into the same container as the 5 supervised asyncio tasks (heartbeat, status_check,
receiver, polling, log_publisher) but run it on its own OS-thread because FastMCP
starts an internal uvicorn ASGI server with its own event loop.
...
def start_mcp_thread() -> threading.Thread | None:
    """Start the CRM MCP server in a daemon thread.

    Returns the running Thread on success, or None if the package is not
    installed or if `CRM_MCP_ENABLED=false`. Errors during MCP startup or
    runtime are logged but never propagate -- the main asyncio loop keeps
    running even if MCP fails (defense-in-depth: a Salesforce auth failure
    should not take down the heartbeat).
    """
